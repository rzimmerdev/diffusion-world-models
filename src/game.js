...
function buildAlphasCumprod(T = T_TOTAL) {
  ...
  for (let i = 0; i < T; i++) {
    const prev = i === 0 ? 1.0 : ac[i-1];
    betas[i] = Math.min(0.999, 1 - ac[i] / prev);
  }
...
async function ddpmInfer(contextFlat, actionVec, steps) {
  ...
  for (let i = 0; i < steps; i++) {
    tSeq.push(Math.round((T_TOTAL - 1) * (1 - i / (steps - 1 || 1))));
  }
