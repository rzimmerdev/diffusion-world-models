...
def _(
    ...
    return (WorldModelAnimation,)


@app.cell
def _(Scene, WorldModelAnimation, mo):
    def show(scene: Scene):
        scene.render()

        path = scene.renderer.file_writer.movie_file_path
        return mo.video(src=str(path), width=900, height=900 * 9 / 16)
    
    show(WorldModelAnimation())
    return
