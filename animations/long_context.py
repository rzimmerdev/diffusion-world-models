...
from manim import (
    Write, FadeIn, Create, GrowArrow, FadeOut,
    LaggedStart, there_and_back, CurvedArrow,
)
from manim import (
    Square, Arrow, DashedLine, Dot, RoundedRectangle,
    Line, Text, Tex, SurroundingRectangle,
)
from manim import UP, DOWN, LEFT, RIGHT, PI
from manim import BOLD, ITALIC
from manim import (
    WHITE,
    GREY_A, GREY_B, GREY_C, GREY_D,
    BLUE_D,
    GREEN_C,
    RED, RED_B, RED_C,
...
def make_mini_frame(agent_pos, noise=(0.0, 0.0), cell_size=0.17, n_cells=3):
    ...
    frame = Square(side_length=total,
                   fill_color=MAZE_BG, fill_opacity=1.0,
    ...
    for i in range(1, n_cells):
        grid.add(Line(
            ...
            stroke_color=MAZE_GRID, stroke_width=0.6,
        ))
        grid.add(Line(
    ...
    for (x1, y1), (x2, y2) in wall_cfg:
        walls.add(Line(
            ...
            stroke_color=MAZE_WALL, stroke_width=2.5,
        ))

    # Agent dot
    cx = frame.get_left()[0]  + (agent_pos[0] + 0.5 + noise[0]) * cell_size
    cy = frame.get_bottom()[1] + (agent_pos[1] + 0.5 + noise[1]) * cell_size
    agent = Dot(point=[cx, cy, 0], radius=cell_size * 0.32,
                color=AGENT_COLOR, stroke_width=1, stroke_color=WHITE)
...
class LongContextComparison(Scene):
    ...
    def construct(self):
        ...
        # ── Title ─────────────────────────────────────────────────────────
        title = Text("Long Context: Forward → Reverse Consistency",
        ...
        lbl_L = Text("Diffusion Baseline", font_size=20, color=DIFFUSION_COLOR, weight=BOLD)
        lbl_L.next_to(panel_L, UP, buff=0.15)

        lbl_R = Text("StateSpaceDiffuser  (Ours)", font_size=20, color=SSM_COLOR, weight=BOLD)
        lbl_R.next_to(panel_R, UP, buff=0.15)

        self.play(
            FadeIn(panel_L), FadeIn(panel_R),
            Write(lbl_L), Write(lbl_R),
            run_time=0.8,
        )
        ...
        self.play(
            Write(fwd_label_L), Write(fwd_label_R),
        ...
        rev_frames_L = self._build_frame_row(
            DIFFUSION_REVERSE, panel_L.get_center(), y_off=-1.55, noise=0.0,
        ...
        self.play(
            Write(rev_label_L), Write(rev_label_R),
        ...
        highlight_box = SurroundingRectangle(
            ...
            color=SSM_COLOR, buff=0.15, corner_radius=0.1,
        )
        ...
        # ── Final message ─────────────────────────────────────────────────
        msg = Tex(
            r"\text{Long-context memory: StateSpaceDiffuser preserves fidelity}",
            r"\text{far from the U-turn -- Diffusion collapses}",
    ...
    def _build_frame_row(self, positions, panel_center, y_off, noise=0.0,
    ...
    def _build_connectors(self, fwd_frames, rev_frames, psnr_values, psnr_color):
        ...
        for i, (fw, rv) in enumerate(zip(fwd_frames, rev_frames)):
            top = fw.get_bottom()
            bot = rv.get_top()
            conn = make_dotted_connector(top, bot,
                                         color=GREY_C if psnr_color == PSNR_BAD else GREY_D)
            connectors.add(conn)
