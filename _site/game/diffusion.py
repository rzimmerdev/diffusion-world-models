...
# ─────────────────────────── constants ────────────────────────────────────────
CELL = 8           # pixels per maze cell (16→8 for speed)
...
class DiffusionWorldModel(nn.Module):
    ...
    def __init__(self, context_len=CONTEXT_LEN, base_ch=16):
        super().__init__()
...
def train(checkpoint_path: str | None, out_path: str, device_str: str, steps: int, lr: float):
    ...
    for epoch in range(max(1, steps // 800)):
        ...
        for s in range(800):
            ...
            if total_step % 200 == 0:
                ...
                print(f"  step {total_step}  loss={loss:.4f}")
    ...
    final_loss = train_on_replay(model, optimizer, replay, device, steps=200)
    print(f"  final (step {total_step})  loss={final_loss:.4f}")


    # ── quick text preview ──────────────────────────────────────────────
    model.eval()
    with torch.no_grad():
        prev_maze = generate_maze(COLS, ROWS)
        prev_pos = [1, 1]
        ctx_win = [maze_to_tensor(prev_maze, prev_pos, goal_pos) for _ in range(CONTEXT_LEN)]
        ctx_stack = torch.stack(ctx_win).view(1, CONTEXT_LEN * 3, H, W).to(device)
        act = torch.zeros(1, 4, device=device); act[0, 0] = 1.0
        pred = ddpm_infer(model, ctx_stack, act, steps=DIFF_STEPS, device=device)
        pred_01 = (pred.squeeze(0).cpu() * 0.5 + 0.5)
        wall_std  = pred_01[:, :CELL, :].std().item() + pred_01[:, -CELL:, :].std().item()
        print(f"  preview  std={pred_01.std().item():.3f}  wall_contrast={wall_std:.3f}  (>0.3 = structure)")
